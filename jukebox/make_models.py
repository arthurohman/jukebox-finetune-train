...
MODELS = {
    ...
    'your_model': ("small_vqvae", "upsampler_level_0", "upsampler_level_1", "my_retrained_prior")
}
...
def save_checkpoint(logger, name, model, opt, metrics, hps):
    with t.no_grad():
        ...
        t.save({'hps': save_hps,
                ...
                'step': logger.iters,
                **metrics}, f'{logger.logdir}/checkpoint_{name}_{logger.iters}.pth.tar')
    return
...
def make_prior(hps, vqvae, device='cuda'):
    ...
    print("PRIOR FIRST: ", prior.__dict__)
    if hps.fp16_params:
